...
def coord_label(r, c):
    """Convert (r,c) -> 'A1' style."""
    letter = chr(ord('A') + c)  # A..S
    number = str(r + 1)
...
def draw_board(screen):
    ...
    pygame.draw.rect(
        screen, BOARD_BG,
        (x0 - 18, y0 - 18, BOARD_W + 36, BOARD_H + 36),
        border_radius=18
    )
...
def draw_stones(screen, board, move_index, show_numbers, move_no_font):
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            if show_numbers:
                k = move_index[r][c]
                if k != 0:
                    txt = move_no_font.render(str(k), True, P_TEXT[p])
...
def draw_topbar(screen, font, small, turn_player, winner, draw_flag, last_move_label):
    ...
    if last_move_label:
        screen.blit(small.render(f"Last move: {last_move_label}", True, MUTED), (MARGIN + 18, MARGIN + 50))

    if not winner and not draw_flag:
        pygame.draw.circle(screen, P_COLORS[turn_player], (MARGIN + 180, MARGIN + 36), 14)
        pygame.draw.circle(screen, (0, 0, 0), (MARGIN + 180, MARGIN + 36), 14, 2)
...
def main():
    pygame.init()
    pygame.display.set_caption("Saminsamok - 3-Player Gomoku (4)")
    ...
    # NEW: toggle button above Restart
    toggle_btn = Button((MARGIN * 2 + BOARD_W + 30, H - MARGIN - 180, PANEL_W - 60, 48), "Move #: ON", font)

    state = "menu"  # menu | game
    board = new_board()
    move_index = new_move_index()
    show_numbers = True  # default ON (for review)
    turn_player = 1
    winner = 0
    draw_flag = False
    hover_rc = None
    ...
    move_no_font = pygame.font.SysFont("Segoe UI", 16, bold=True)

    def reset_game(to_menu=False):
        nonlocal board, move_index, turn_player, winner, draw_flag, hover_rc
        nonlocal moves_played, last_move_label, state
        board = new_board()
        move_index = new_move_index()
        turn_player = 1
        winner = 0
    ...
    while running:
        ...
        if state == "menu":
            ...
            toggle_btn.handle_motion((mx, my))
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
                if state == "menu":
                    ...
                    # Toggle
                    if toggle_btn.clicked((mx, my)):
                        show_numbers = not show_numbers
                        toggle_btn.label = "Move #: ON" if show_numbers else "Move #: OFF"
                        continue
                    ...
                    moves_played += 1
                    move_index[r][c] = moves_played  # store move number
        ...
        if state == "menu":
            title = big.render("Saminsamok", True, TEXT)
            screen.blit(title, title.get_rect(center=(W // 2, H // 2 - 150)))
            title2 = big.render("(3-Player Gomoku)", True, TEXT)
            screen.blit(title2, title2.get_rect(center=(W // 2, H // 2 - 110)))
            ...
            draw_board(screen)
            draw_stones(screen, board, move_index, show_numbers, move_no_font)
