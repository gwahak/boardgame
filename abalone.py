...
def try_move(board: Dict[Pos, int], cells: Set[Pos], current: int, selected: List[Pos], d: Pos,
    ...
    if board[next_pos] != opp:
        return False, "Inline blocked by your own marble."

    # count opponent chain
    opp_chain = []
    cur = next_pos
    while in_board(cur, cells) and board[cur] == opp:
    ...
    if opp_n > 2:
        return False, "Can push at most 2 opponent marbles."
    ...
    if not in_board(after, cells):
        pushed_out[current] += 1
        for i in range(opp_n - 1, 0, -1):
            board[add(opp_chain[i - 1], d)] = opp
        board[opp_chain[0]] = EMPTY
    else:
        board[after] = opp
        for i in range(opp_n - 1, 0, -1):
            board[opp_chain[i]] = opp
        board[opp_chain[0]] = EMPTY
...
def main():
    ...
    P1_FILL = (210, 70, 70)     # red-ish
    P2_FILL = (70, 110, 210)    # blue-ish
    STONE_EDGE = (25, 25, 25)
    ...
    current = P1
    selected: List[Pos] = []
    pushed_out = {P1: 0, P2: 0}
    winner: Optional[int] = None
    msg = f"Mode: {mode_key} (click-to-move)."

    # buttons in game
    restart_btn = Button((W - 280, 64, 120, 40), "Restart")
    newgame_btn = Button((W - 150, 64, 120, 40), "New Game")

    def restart_same_mode():
        nonlocal current, selected, winner, msg, players, win_push
        players, win_push = load_mode(mode_key, board, cells)
        selected = []
        pushed_out[P1] = 0
        pushed_out[P2] = 0
    ...
    while running:
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                ...
                elif ENABLE_KEY_MOVES and (winner is None) and (e.key in KEY_TO_DIR):
                    d = KEY_TO_DIR[e.key]
        ...
        for (q, r), pid in board.items():
            if pid == EMPTY:
                continue
            cx, cy = axial_to_pixel(q, r, size, origin)
            color = P1_FILL if pid == P1 else P2_FILL
        ...
        # top info
        top_y = 18
        header = f"Mode: {mode_key}   Turn: P{current}   P1 pushed:{pushed_out[P1]}  P2 pushed:{pushed_out[P2]}  Win:{win_push}"
        screen.blit(font_big.render(header, True, (20, 20, 20)), (20, top_y))

        if winner is not None:
            win_txt = f"WINNER: P{winner}"
            screen.blit(font_big.render(win_txt, True, (10, 10, 10)), (20, top_y + 34))
